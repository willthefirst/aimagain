...
from sqlalchemy import (
    create_engine, MetaData, Table, Column, String, Text, DateTime, Boolean, ForeignKey,
...
class User(Base):
    ...
    is_online = Column(Boolean, nullable=False, server_default=sqlalchemy.sql.expression.false())

    # Relationships
    # Conversations created by this user
    created_conversations = relationship("Conversation", back_populates="creator", foreign_keys="[Conversation.created_by_user_id]")
    # Messages sent by this user
    messages = relationship("Message", back_populates="sender", foreign_keys="[Message.created_by_user_id]")
    ...
    sent_invitations = relationship("Participant", back_populates="inviter", foreign_keys="[Participant.invited_by_user_id]")
...
class Conversation(Base):
    ...
    creator = relationship("User", back_populates="created_conversations", foreign_keys=[created_by_user_id])
...
class Message(Base):
    ...
    invitations_as_initial = relationship("Participant", back_populates="initial_message", foreign_keys="[Participant.initial_message_id]")
...
class Participant(Base):
    ...
    inviter = relationship("User", back_populates="sent_invitations", foreign_keys=[invited_by_user_id])
    initial_message = relationship("Message", back_populates="invitations_as_initial", foreign_keys=[initial_message_id])

    # Constraints
    __table_args__ = (
        UniqueConstraint('user_id', 'conversation_id', name='uq_participant_user_conversation'),
        CheckConstraint("status IN ('invited', 'joined', 'rejected', 'left')", name='ck_participant_status'),
    )
