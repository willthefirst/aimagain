...
| `POST /users/me/favorites/{provider_id}`, `DELETE /users/me/favorites/{provider_id}`, `GET /users/me/favorites` | `src/entities/favorites/route.py` | M:N edge add/remove — mounted via the spec-driven `mount_edge_routes(...)` helper. `FAVORITE_ENTITY` carries `edge_audit` + `relation: M2NRelation`. The favorites entity has been migrated into `src/entities/`; see [`src/entities/favorites/`](../../entities/favorites/). |
