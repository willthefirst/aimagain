...
| **posts.py**       | Posts                | Post listing, detail, create, partial update, hard delete, and per-kind create/edit form pages | `GET /posts`, `GET /posts/form?kind={note,client_referral}`, `GET /posts/{id}`, `GET /posts/{id}/form`, `POST /posts`, `PATCH /posts/{id}`, `DELETE /posts/{id}` | PostRepository        |
...
- `posts.py` - Post listing, detail, create, partial update, hard delete, and per-kind HTML form pages. `GET /posts/form?kind={note,client_referral}` selects the create-form template (`Literal` validator → 422 on unknown kinds); `GET /posts/{id}/form` selects the edit-form template from the persisted `post.kind`. `POST /posts` accepts a kind-discriminated body (defaults to `note` for backward-compat); `PATCH /posts/{id}` rejects payloads whose `kind` doesn't match the persisted post.
...
- `test_posts.py` — `GET /posts`, `GET /posts/{id}`, `POST /posts`, `PATCH /posts/{id}`, `DELETE /posts/{id}`, `GET /posts/form?kind={note,client_referral}`, and `GET /posts/{id}/form` (covers `posts.py`). Exercises both kinds end-to-end: per-kind create/list/detail/edit-form/PATCH/DELETE, the kind-mismatch 400 on PATCH, owner-or-admin authorization on PATCH and DELETE, the `extra="forbid"` scrub of `owner_id`, route-ordering checks, audit-row before/after assertions, and `_owner_actions.html` partial visibility on the detail page. Pact contract pairs for the note forms and the owner-actions Delete button live under [`tests/test_contract/`](../../../tests/test_contract/README.md); per-kind contract pairs for client_referral forms are deferred until the form schema stabilizes.
