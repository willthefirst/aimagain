...
| **provider_profiles.py** | Provider profiles | Profile + credential sub-resource CRUD (JSON API, form-encoded bodies) plus the create- and edit-form HTML pages. Public listing; the rest require auth and authorize through the logic-layer's owner/admin guard. Mutation responses set `HX-Redirect` back to `/{id}/form` so the edit page is the post-submit landing for every profile + sub-resource mutation. | `GET /provider-profiles` (public, `?license_type=&issuing_state=` filters), `POST /provider-profiles`, `GET /provider-profiles/me`, `GET /provider-profiles/form`, `GET /provider-profiles/{id}`, `GET /provider-profiles/{id}/form`, `PATCH /provider-profiles/{id}`, `DELETE /provider-profiles/{id}`, plus `POST/PATCH/DELETE /provider-profiles/{id}/{licensures,educations,certifications}/...` | ProviderProfileRepository |
...
- `provider_profiles.py` - Provider profile + three credential sub-resources (licensure, education, certification). JSON API on form-encoded bodies (the codebase standard); responses are `JSONResponse` with `HX-Redirect`/`Location` headers. Listing is public; everything else requires `current_active_user`. Authorization (owner-or-admin) is delegated entirely to the logic layer's `_assert_can_mutate` — routes are thin adapters that bind params, validate via per-schema `TypeAdapter`, and delegate to the matching `handle_*` function. Reuses `parse_form_to_payload` from `posts.py`. `GET /provider-profiles/form` returns the create-form HTML page (registered before `/{profile_id}` so the literal `form` is not parsed as a UUID); the form posts to `POST /provider-profiles`. `GET /provider-profiles/{id}/form` returns the edit-form HTML page (owner-or-admin guard via `handle_get_provider_profile_edit_form`). All profile + sub-resource mutation responses (POST/PATCH/DELETE except parent DELETE) set `HX-Redirect: /provider-profiles/{id}/form` so the user lands back on the edit page; parent DELETE redirects to `/provider-profiles`.
