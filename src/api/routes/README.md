...
- `test_posts.py` — `GET /posts`, `GET /posts/{id}`, `POST /posts`, `PATCH /posts/{id}`, `DELETE /posts/{id}`, `GET /posts/form?kind={note,client_referral,provider_availability}`, and `GET /posts/{id}/form` (covers `posts.py`). Exercises every kind end-to-end: per-kind create/list/detail/edit-form/PATCH/DELETE, the kind-mismatch 400 on PATCH (asserts state-unchanged + no audit row), owner-or-admin authorization on PATCH and DELETE, the `extra="forbid"` scrub of `owner_id`, route-ordering checks, audit-row before/after assertions, and `_owner_actions.html` partial visibility on the detail page. Pact contract pairs for the note forms and the owner-actions Delete button live under [`tests/test_contract/`](../../../tests/test_contract/README.md); per-kind contract pairs for the non-note kinds are deferred until their form schemas stabilize.
