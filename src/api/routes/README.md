...
| ------------------ | -------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| **users.py**       | User data            | User listing, detail, admin actions, owned-provider-profile list | `GET /users`, `GET /users/{id}`, `GET /users/{id}/provider-profiles` (self or admin), `PUT /users/{id}/activation` (admin), `DELETE /users/{id}` (admin) | UserRepository, ProviderProfileRepository |
...
| **provider_profiles.py** | Provider profiles | Profile + credential sub-resource CRUD (form-encoded mutation bodies, JSON + `HX-Redirect` mutation responses) plus public HTML directory listing, HTML detail (`GET /{id}`), and create/edit form pages. Public listing; the rest require auth and authorize through the logic-layer's owner/admin guard. Mutation responses set `HX-Redirect` back to `/{id}/form` so the edit page is the post-submit landing for every profile + sub-resource mutation. The "current user's profiles" view lives at `/users/me/provider-profiles` (see `me.py`), not on this resource. | `GET /provider-profiles` (public HTML, `?license_type=&issuing_state=` filters), `POST /provider-profiles`, `GET /provider-profiles/form`, `GET /provider-profiles/{id}` (HTML detail), `GET /provider-profiles/{id}/form`, `PATCH /provider-profiles/{id}`, `DELETE /provider-profiles/{id}`, plus `POST/PATCH/DELETE /provider-profiles/{id}/{licensures,educations,certifications}/...` | ProviderProfileRepository |
| **me.py**          | Current user context | User profile, current-user JSON, current-user provider profiles | `GET /users/me`, `GET /users/me/profile`, `GET /users/me/provider-profiles`                                       | Auth, ProviderProfileRepository |
...
- `provider_profiles.py` - Provider profile + three credential sub-resources (licensure, education, certification). Form-encoded mutation bodies (the codebase standard); mutation responses are `JSONResponse` with `HX-Redirect`/`Location` headers. Read routes render HTML: `GET /provider-profiles` renders `provider_profiles/list.html` (public; the template includes a license-type / issuing-state filter form whose active values are echoed back as `selected_*` context keys), and `GET /provider-profiles/{id}` renders `provider_profiles/detail.html` (the `Edit profile` link is gated to owner-or-admin in the template using `current_user`). Listing is the only public read; everything else requires `current_active_user`. Authorization (owner-or-admin) is delegated entirely to the logic layer's `_assert_can_mutate` — routes are thin adapters that bind params, validate via per-schema `TypeAdapter`, and delegate to the matching `handle_*` function. Form-encoded bodies and the 422 translation flow through `parse_form_to_payload` and `validate_or_422` from `src/api/common/forms.py`. `GET /provider-profiles/form` returns the create-form HTML page (registered before `/{profile_id}` so the literal `form` is not parsed as a UUID); the form posts to `POST /provider-profiles`. `GET /provider-profiles/{id}/form` returns the edit-form HTML page (owner-or-admin guard via `handle_get_provider_profile_edit_form`). All profile + sub-resource mutation responses (POST/PATCH/DELETE except parent DELETE) set `HX-Redirect: /provider-profiles/{id}/form` so the user lands back on the edit page; parent DELETE redirects to `/provider-profiles`. The "list profiles owned by user X" view is an ownership-subresource of `users` and lives at `GET /users/{id}/provider-profiles` (in `users.py`) with a `/users/me/provider-profiles` alias (in `me.py`) — see `RESOURCE_GRAMMAR.md` on ownership subresources.
- `me.py` - Current user shortcuts: `GET /users/me` (JSON), `GET /users/me/profile` (HTML user-identity page), and `GET /users/me/provider-profiles` (HTML list of provider profiles owned by the current user — alias for `GET /users/{my_id}/provider-profiles`). The provider-profile alias delegates to `handle_list_user_provider_profiles` so its auth/template/empty-state behavior is identical to the user-scoped form.
