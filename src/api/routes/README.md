...
| ------------------ | -------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| **users.py**       | User data            | User listing, detail, admin actions, owned-provider list | `GET /users`, `GET /users/{id}`, `GET /users/{id}/provider-profiles` (self or admin), `PUT /users/{id}/activation` (admin), `DELETE /users/{id}` (admin) | UserRepository, ProviderRepository |
...
- `me.py` - Current user shortcuts: `GET /users/me` (JSON), `GET /users/me/profile` (HTML user-identity page), and `GET /users/me/provider-profiles` (HTML list of providers owned by the current user — alias for `GET /users/{my_id}/provider-profiles`). The alias delegates to `handle_list_user_providers` so its auth/template/empty-state behavior is identical to the user-scoped form.
