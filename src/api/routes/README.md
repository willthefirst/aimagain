...
| **provider_profiles.py** | Provider profiles | Profile + credential sub-resource CRUD (form-encoded bodies, mutations return JSON + `HX-Redirect`) plus HTML detail (`GET /{id}` and `GET /me`) and create/edit form pages. Public listing; the rest require auth and authorize through the logic-layer's owner/admin guard. Mutation responses set `HX-Redirect` back to `/{id}/form` so the edit page is the post-submit landing for every profile + sub-resource mutation. | `GET /provider-profiles` (public, `?license_type=&issuing_state=` filters), `POST /provider-profiles`, `GET /provider-profiles/me` (HTML detail), `GET /provider-profiles/form`, `GET /provider-profiles/{id}` (HTML detail), `GET /provider-profiles/{id}/form`, `PATCH /provider-profiles/{id}`, `DELETE /provider-profiles/{id}`, plus `POST/PATCH/DELETE /provider-profiles/{id}/{licensures,educations,certifications}/...` | ProviderProfileRepository |
