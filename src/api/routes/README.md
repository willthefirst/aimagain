...
| **posts.py**       | Posts (polymorphic — kind-discriminated) | Unified timeline (every kind), detail, create (discriminated body), partial update (per-kind PATCH for both kinds), hard delete, and both form pages. Each kind has its own edit template selected via a route-layer lookup. | `GET /posts`, `GET /posts/form`, `GET /posts/{id}`, `GET /posts/{id}/form`, `POST /posts`, `PATCH /posts/{id}`, `DELETE /posts/{id}` | PostRepository |
...
- `posts.py` - Polymorphic posts: unified timeline, detail, create (kind-discriminated body), per-kind PATCH, hard delete, and both HTML form pages (`GET /posts/form`, `GET /posts/{id}/form`). The edit-form route picks a per-kind template via a small lookup.
...
- `test_posts.py` — `GET /posts`, `GET /posts/{id}`, `POST /posts` (kind-discriminated body), `PATCH /posts/{id}` (both kinds), `DELETE /posts/{id}`, `GET /posts/form`, and `GET /posts/{id}/form` (covers `posts.py`). Exercises both kinds end-to-end: discriminator dispatch, per-kind required fields including `accepting_new_clients` boolean coercion, child-table cascade on delete, owner-or-admin authorization on PATCH/DELETE, kind-mismatch and at-least-one-field rules on PATCH, the `extra="forbid"` scrub of `owner_id`, route-ordering checks, audit-row snapshots that include `kind` + per-kind fields, per-kind edit forms rendering with the hidden discriminator, and `_owner_actions.html` partial visibility. Pact contract pairs for the create/edit/owner-actions flows live under [`tests/test_contract/`](../../../tests/test_contract/README.md) (`provider_availability`-specific create/edit pacts are a deferred follow-up — see that README).
