...
| **posts.py**       | Posts (polymorphic — kind-discriminated) | Unified timeline (every kind), detail, create (discriminated body), partial update (per-kind PATCH; only `client_referral` has editable fields so far), hard delete, and both form pages. The edit form 404s for kinds without an Update variant. | `GET /posts`, `GET /posts/form`, `GET /posts/{id}`, `GET /posts/{id}/form`, `POST /posts`, `PATCH /posts/{id}`, `DELETE /posts/{id}` | PostRepository |
...
- `posts.py` - Polymorphic posts: unified timeline, detail, create (kind-discriminated body), per-kind PATCH, hard delete, and both HTML form pages (`GET /posts/form`, `GET /posts/{id}/form` — the latter 404s for kinds without an Update variant).
...
- `test_posts.py` — `GET /posts`, `GET /posts/{id}`, `POST /posts` (kind-discriminated body), `PATCH /posts/{id}` (client_referral only), `DELETE /posts/{id}`, `GET /posts/form`, and `GET /posts/{id}/form` (covers `posts.py`). Exercises both kinds end-to-end: discriminator dispatch, per-kind required fields, child-table cascade on delete, owner-or-admin authorization on PATCH/DELETE, kind-mismatch and at-least-one-field rules on PATCH, the `extra="forbid"` scrub of `owner_id`, route-ordering checks, audit-row snapshots that include `kind` + per-kind fields, edit form rendering with the hidden discriminator, and `_owner_actions.html` partial visibility (Edit link only for kinds with editable fields). Pact contract pairs for both forms and the owner-actions Delete button live under [`tests/test_contract/`](../../../tests/test_contract/README.md).
