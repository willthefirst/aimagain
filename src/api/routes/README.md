...
| **provider_profiles.py** | Provider profiles | Profile + credential sub-resource CRUD (JSON API, form-encoded bodies) plus the create-form HTML page. Public listing; the rest require auth and authorize through the logic-layer's owner/admin guard. | `GET /provider-profiles` (public, `?license_type=&issuing_state=` filters), `POST /provider-profiles`, `GET /provider-profiles/me`, `GET /provider-profiles/form`, `GET /provider-profiles/{id}`, `PATCH /provider-profiles/{id}`, `DELETE /provider-profiles/{id}`, plus `POST/PATCH/DELETE /provider-profiles/{id}/{licensures,educations,certifications}/...` | ProviderProfileRepository |
