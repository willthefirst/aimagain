...
Jinja globals follow the same direction: [`rendering/templating.py`](rendering/templating.py) exposes only the framework's own `field_spec` global plus a `register_template_globals(**kwargs)` API. Domain enums, per-kind create schemas, and view helpers are registered from [`../domain/template_globals.py`](../domain/template_globals.py) at module load. App entry points (`src/main.py`, the contract-test consumer server) import that module to trigger registration before any template renders.
