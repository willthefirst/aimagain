...
**Lock guarantee for hand-written anchors.** Every `<a>` that points at a gated entity's collection list must go through `entity_link(name, label, …)` in [`_shared/_locked.html`](_shared/_locked.html). A bare `<a href="{{ entity_url('user') }}">…</a>` (where `user` has `ReadPolicy.lock_reason`) is a lint error — see [`scripts/dev/template_route_check.py`](../../../scripts/dev/template_route_check.py), which scans every `.html` file for the bug class. The lint is scoped to `entity_url('GATED')` calls *without* `id=`/`subresource=` args — only the collection-list path triggers the read-policy gate; detail / form / subresource paths don't 403 and don't need the macro. Gated entities are derived from the live entity registry, so new `ReadPolicy.lock_reason` declarations auto-extend the lint.
