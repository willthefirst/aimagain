...
- `_picker.html` — `picker(options)`: the "choose your path" card grid. Each option is a dict of `href` / `heading` / `description`; the macro emits one `<a><hgroup><h2>…</h2><p>…</p></hgroup></a>` card per option. Funnels the user to a kind- or type-specific sub-form before any fields appear — when a multi-option entry point lands without its selecting query param. Used by `/posts/form` (`?kind=`) and `/organizations/form` (`?type=`). Extracted from the posts pattern (#704).
