...
**Toolbar / action bar** is the zone bar for page-scoped controls. Every page that needs it renders the same two-zone `<div class="toolbar">` shell (see `_shared/_toolbar.html`): an optional search link on the left and the page-action `<menu class="toolbar-right">` (Create, Edit, Delete, Favorite, Export...) pushed to the right edge via `margin-left: auto`. The action cluster is a `<menu>` — HTML's native "list of commands" element — so the page's **primary resource actions** are marked up as the toolbar a screen reader / browser already expects. Each action is a `<li>` child of the menu. List pages with `routes.search` populate the left zone with a search link opening `/<collection>/search`; the link doubles as the active-filter summary, reading "Type: Seeking, Description: needle, +2 more filters" (first two inline, the rest collapsed) with a sibling `Clear all` link when any filter is active, and `Search <collection>` when none are. Detail pages leave the left zone empty and compose the right zone with the resource's action partial (`_owner_actions.html`, `_admin_actions.html`) or open-coded `<li><button>…</button></li>` items — the actions still land at the right edge because the right-zone rule doesn't depend on the left zone being occupied. Pages without page-scoped controls leave the block empty.
...
- `_toolbar.html` — `page_toolbar(active_filters=(), search_url=None, clear_all_url=None, collection_label="")`: the toolbar shell that both `views/list.html` and `views/detail.html` compose. Emits a two-zone `<div class="toolbar">` strip — optional search link on the left (omitted when `search_url` is `None`, as on detail pages), and the right-zone action `<menu class="toolbar-right">` (caller body is `<li>` items) pushed to the right edge via `margin-left: auto`. The search link does double duty: when no filters are active it reads ``Search <collection>``; when filters are active it summarizes the first two inline and collapses the rest to ``+N more filters``, with a sibling ``Clear all`` link beside it. No separate active-filter strip — the inline summary replaces it. On list pages the context comes from `handle_list` (`active_filters`, `search_url`, `clear_all_url`, `collection_label`); detail pages pass no args.
