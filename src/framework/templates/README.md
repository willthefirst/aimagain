...
- `_picker.html` — `picker(options)`: the "choose your path" card stack. Each option is a dict of `href` / `heading` / `description`; the macro emits one `<article class="picker-option"><header><h2><a>…</a></h2></header><p>…</p></article>` per option. Articles are block-level so options stack as rows on every viewport (no grid). No custom CSS — Pico's default `<article>` chrome (border, padding, header band) and default link/heading styling carry the look. Two usages, one component: a *discriminator/type picker* funnels to a kind-/type-specific sub-form of the **same** resource via a selecting query param (`/posts/form` `?kind=`); a *dispatching picker* points its cards at **other** resources' create forms (e.g. an identity picker → `/clinicians/form` and `/organizations/form`). The macro only takes `{href, heading, description}` (plus an optional `selected` flag that renders the heading as plain text without an `<a>` and tags the card with `aria-current="page"` plus a checkmark glyph) — self- vs cross-resource hrefs are the caller's business. Extracted from the posts pattern (#704); pinned by [`_shared/test_picker.py`](_shared/test_picker.py).
