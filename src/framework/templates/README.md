...
**Capability-aware back affordance.** Both the detail view's collection segment and the mount-injected `_breadcrumb_items` carry the third tuple element — a `REASON_*` lock code from `capabilities.py`, or `None`. When the back target's entity declares `ReadPolicy.lock_reason` and the viewer fails `read_policy.can_read`, the breadcrumb macro emits a `data-locked-cta` popover trigger instead of an `<a href>` — so clicking `< Users` from `/users/me` opens the "Get provider network access" CTA rather than landing on a 403. The Jinja-global `entity_lock_reason(name)` (see [`../rendering/route_urls.py`](../rendering/route_urls.py)) resolves the reason for any entity from template context; the `entity_link(name, label, …)` macro in [`_shared/_locked.html`](_shared/_locked.html) wraps the same gate for hand-written `<a>` sites (empty-state "Browse X" links, etc.).
