...
- `_picker.html` — `picker(options)`: the "choose your path" card grid. Each option is a dict of `href` / `heading` / `description`; the macro emits a `<div class="picker">` grid wrapping one `<a class="picker-option"><hgroup><h2>…</h2><p>…</p></hgroup></a>` card per option. The `.picker` / `.picker-option` styling lives in `framework.css` — bordered cards with a link-coloured heading and muted (non-underlined) description, so the chooser reads as the page's primary action rather than a stack of underlined links. Funnels the user to a kind- or type-specific sub-form before any fields appear — when a multi-option entry point lands without its selecting query param. Used by `/posts/form` (`?kind=`), `/organizations/form` (`?type=`), and the `/profile/identity` setup step (`?path=`). Extracted from the posts pattern (#704).
