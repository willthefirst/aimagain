...
- `form_error_handler.py` — route decorator. Declarative version of "wrap the body in try/except → form_rerender on HX-Request". Takes `template=`, `prefill_fields=`, and a `handlers={ExceptionType: (e -> FormError)}` dict; on a matched exception with `HX-Request: true` it short-circuits to `form_rerender`; otherwise it re-raises so the outer `handle_route_errors` does its usual JSON-4xx translation. Used by `register_request_handler`; future auth/form routes should reach for it before hand-rolling try/except. Note: `auth_pages.post_login` doesn't use it because its rerender path is `user is None` (procedural), not exception-based — see PR #3 in the form-error generalization sequence for the unification.
