...
- `test_provider_models.py` — direct DB-layer coverage of the cluster: per-user FK behavior, the cascade from a profile down to its credential lists (raw-SQL `DELETE` on the parent proves FK CASCADE fires, not just the ORM cascade), and `IntegrityError` on each enum CHECK rejection.
