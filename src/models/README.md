...
- `provider_availability_detail.py` - `kind='provider_availability'` detail; 1:1 with `posts` via `post_id`. Columns cover the scalar fields from `notes/forms_spec.md` Form 2 plus the `desired_times` and `services` JSON multi-selects (`services` is required-min-1 on the wire here); enum columns CHECK against `post_enums.py` (JSON columns don't — vocabularies are enforced on the wire by Pydantic).
