...
- `provider_profile.py` - Long-lived directory profile for a provider (one per user, enforced by `UniqueConstraint` on `user_id`). Owns `ProviderLicensure` / `ProviderEducation` / `ProviderCertification` rows via `cascade="all, delete-orphan"` + FK CASCADE. Distinct from `provider_availability_detail.py` (per-`Post`); this row describes the provider themselves.
