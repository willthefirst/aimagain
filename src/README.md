...
6. **Logic** — write bespoke `handle_*` functions in `src/logic/<entity>/<entity>_processing.py` *only* for verbs that have rules the generic framework can't subsume (custom auth predicates beyond `write_authz`, multi-step writes, edge mutations). For the standard create / update / delete shape, bind a factory-built handler from [`src/logic/_generic.py`](logic/README.md) via `make_<verb>_handler(<ENTITY>_ENTITY)`. Raise the API exceptions from [`src/api/common/exceptions.py`](api/common/exceptions.py) directly (`NotFoundError`, `ForbiddenError`, etc.) — see [Error handling](#error-handling).
7. **Route** — create `src/api/routes/<entity>.py`, build the factory-bound handlers as named module attributes (`_handle_<verb>_<entity>`), and call `mount_entity(router, <ENTITY>_ENTITY, handlers={...}, owned_subentities=(...))` once. See [`api/routes/README.md`](api/routes/README.md#implementation-patterns).
