...
7. **Route** — create `src/api/routes/<entity>.py` and call `mount_entity(router, <ENTITY>_ENTITY, handlers={...}, owned_subentities=(...), module=__name__)` once. The dispatcher stitches factory-built handlers onto the route module as `_handle_<verb>_<entity>` so contract-test patches resolve through it. See [`api/routes/README.md`](api/routes/README.md#implementation-patterns).
