...
- Parent-level shared tier:
  ...
  - `_generic.py` — generic framework handlers driven by `EntitySpec`. Covers the standard CRUD verbs — `handle_create(spec, ...)`, `handle_update(spec, ...)`, `handle_delete(spec, ...)` (each performs the standard load → optional parent-FK check → write_authz → audited mutation ritual) — and the read surface: `handle_detail(spec, ..., extras=None, extra_kwargs=None)` (load → optional `can_edit` from `spec.can_write` → entity-specific extras callable for viewer-pair / related-collection / projection work) and `handle_get_edit_form(spec, ...)` (load → write_authz → context dict, including per-kind `template_name` for polymorphic entities). Each ships with a `make_<verb>_handler(spec)` factory (`make_create_handler`, `make_update_handler`, `make_delete_handler`, `make_detail_handler`, `make_edit_form_handler`) that synthesizes a typed signature for the mount layer to introspect. Entities with the standard shape mount factory-built handlers from here; entities with genuinely entity-specific viewer state (`is_favorited`, related collections, private-field projection) supply a pure-function `extras` callable to the detail factory; entities with bespoke mutation rules keep custom `handle_*` functions in their cluster. See the [`mount_entity` dispatcher](../api/common/README.md#mount_entity-dispatcher) for how the factory output gets wired.
