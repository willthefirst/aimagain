...
| **post_processing.py**    | Post operation coordination (kind-discriminated) | list posts (unified timeline across kinds), get post detail, create post (dispatches the discriminated `PostCreate` payload to the right `Post` subclass, server-sets `owner_id`, writes audit row carrying `kind` + per-kind fields, commits), update post (owner-or-admin guard; rejects body-`kind` ↔ post-`kind` mismatch with 400; before/after snapshot; commits), delete post (owner-or-admin guard, audit row recorded before the delete fires with `after=None`, commits — child row cascades via FK), build create-form context, build edit-form context (per-kind template selection lives in the route layer). |
