...
- `test_audit_discipline.py` — static AST check that fails if any `handle_*` in `*_processing.py` calls `.commit()` without `record_audit(...)` or `record_audit_for(...)`. Enforces `RESOURCE_GRAMMAR.md:135` so a future contributor can't silently skip the audit row. Opt out with `audit-discipline-ignore: <reason>` in the handler's docstring.
