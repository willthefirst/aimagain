...
- Parent-level shared tier:
  ...
  - `_generic.py` — generic framework handlers for the standard CRUD verbs, driven by `EntitySpec`. Provides `handle_create(spec, ...)`, `handle_update(spec, ...)`, `handle_delete(spec, ...)` (each performs the standard load → optional parent-FK check → write_authz → audited mutation ritual), plus factory functions `make_create_handler(spec)`, `make_update_handler(spec)`, `make_delete_handler(spec)` that synthesize signatures for the mount layer to introspect. Entities with the standard CRUD shape mount factory-built handlers from here; entities with bespoke rules keep custom `handle_*` functions in their cluster. See the [`mount_entity` dispatcher](../api/common/README.md#mount_entity-dispatcher) for how the factory output gets wired.
