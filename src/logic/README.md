...
| **provider_profile_processing.py** | Provider-profile + credential sub-table coordination | list / get-detail / get-my-detail / create / update / delete profile (owner-or-admin guard on mutations, single audit row per top-level mutation; create accepts inline licensures/educations/certifications and the `CREATE_PROVIDER_PROFILE` snapshot embeds them; delete cascades sub-rows in the DB and emits one `DELETE_PROVIDER_PROFILE` row whose `before` captures the full nested state). Plus create/update/delete handlers for each of the three credential sub-tables, each one verifying that the URL's `profile_id` matches the sub-row's `profile_id` (404 on mismatch). `handle_get_provider_profile_detail` and `handle_get_my_provider_profile_detail` return the read-only detail-page template context (sub-rows eager-loaded via `lazy="selectin"`); `handle_get_provider_profile_form` builds the create-form template context (no DB read; mirror of `handle_get_post_form`); `handle_get_provider_profile_edit_form` loads the profile and enforces the owner-or-admin guard for the edit page. New `AuditAction` values for all twelve mutations live in [`audit.py`](audit.py). |
