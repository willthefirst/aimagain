...
- `index_filters.html` — `index_filters(filters, action, values, target="#index-region")`: filter form for an index page. Reads the `filters` tuple `handle_list` echoes into context (`Filter` instances declared on `EntitySpec.filters`; see [`../../framework/dispatch/filters.py`](../../framework/dispatch/filters.py)) and renders one control per filter (`<input type="search">` for `TextFilter`, `<select>` / `<select multiple>` for `ChoiceFilter`). HTMX submits on change with `hx-target` set to the index region (`#index-region` by default) so filtering is live; the form is also a working no-JS GET form. Live today on `/posts`; the other list pages still use the legacy single-`QueryParam` shape and migrate when their filter set grows.
