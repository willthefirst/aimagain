...
`index_table` also supports `header_kwargs={}` for headers that vary by page state. Not every list uses the table shape — `/posts` renders a dense Craigslist-shaped `<ul id="posts-list">` via `posts/_item.html::post_item(post, active_kind=None)` instead, since the polymorphic kinds need a description-led row (kind chip + lead text + meta tail) the table's fixed columns can't express. Row styling and bullet/border behavior live in `base.html` under `ul#posts-list`.
