...
- `list.html` — listing page (`GET /posts`). Goes through the shared `_shared/index_table.html` macro with the column macros in `_columns.html`. Tabs (All / Seeking / Offering) drive the `?kind=` filter and are server-rendered from the `POST_KINDS` registry; the `kind` query param is declared on `POST_ENTITY.filters` so FastAPI 422s an unknown value. The `#posts-table tbody tr` selector wraps each row. The "New X" links at the top are rendered from `post_kinds` (a list of `PostKindSpec`, passed in by `POST_ENTITY.static_context`), so adding a registered kind automatically adds its create-form link.
- `_columns.html` — `post_headers(active_kind=None)`, `post_row(post, active_kind=None)`, `post_kind_tabs(active_kind, post_kinds)`. The `Type` column is hidden when `active_kind is not None` (a single-kind tab is active). The row branches on `post.kind` to render the right primary cell (CR: literal "Client referral"; PA: linked provider's `practice_name`) and normalizes Location / Population / Insurance across kinds.
...
3. Add a `{% elif post.kind == "<kind>" %}` branch in `_columns.html` (`post_row` primary cell + per-kind data lookup) and `detail.html` (full read-view body). Add the tab label override to `post_kind_tabs` if "Seeking"/"Offering" don't fit. The "New X" link in `list.html` and the tab itself follow automatically via the registry-driven loop.
