...
- `list.html` — listing page (`GET /posts`). Newest-first; each post renders as a Pico `<article>` card (title → 1-line description excerpt → chip row of structured fields → kind label + author). Branches on `post.kind` to pick the right title source (CR: literal "Client referral"; PA: linked provider's `practice_name`) and the right chip set (CR: age groups, languages, insurance; PA: age groups, languages, in/out-of-network posture, sliding scale). The `#post-list > li` selector wraps each card so the listing tests keep working. The "New X" links at the top are rendered from `post_kinds` (a list of `PostKindSpec` from [`src/domain/models/posts/post_kinds.py`](../../models/posts/post_kinds.py), passed in by `handle_list_posts`), so adding a registered kind automatically adds its create-form link.
...
3. Add a `{% elif post.kind == "<kind>" %}` branch in `list.html` (card title + chip row) and `detail.html` (full read-view body). (The "New X" link in `list.html` follows automatically via the registry-driven loop.)
