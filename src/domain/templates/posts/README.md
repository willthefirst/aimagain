...
- `list.html` — listing page (`GET /posts`). Goes through the shared `_shared/index_table.html` macro with the column macros in `_columns.html`. The shared `_shared/index_filters.html` macro renders the filter form above the table; controls come from `POST_ENTITY.filters` (a tuple of `Filter` instances — see [`src/framework/dispatch/filters.py`](../../../framework/dispatch/filters.py)) and HTMX swaps `#index-region` on change. Active filters today: `?kind=` (single-select `<select>` rendered from the `POST_KINDS` registry; still 422s on an unknown value via `ChoiceFilter.value_type=Literal[...]`) and `?q=` (free-text search across both detail tables' `description` columns). The `#posts-table tbody tr` selector wraps each row. The "New X" links at the top are rendered from `post_kinds` (a list of `PostKindSpec`, passed in by `POST_ENTITY.static_context`).
