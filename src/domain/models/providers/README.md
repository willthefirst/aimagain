# Providers cluster: directory entry + credential sub-records

This subdirectory holds the SQLAlchemy models for the long-lived clinician directory entry and its three credential sub-record types. The parent layer's conventions (BaseModel inheritance, FK-relationship coverage, migration workflow) live in [`../README.md`](../README.md); this README covers what's specific to the directory-entry cluster.

## Model-vs-UI vocabulary

The Python identifiers in this cluster — class name `Provider`, file `provider.py`, directory `providers/`, FK column `provider_id`, repo `ProviderRepository` — all use "provider". The user-facing surface (URL family, page labels, entity registry name, audit `resource_type`) uses "clinician" after #642 PR 4. The split is deliberate: the rename was scoped to URLs/templates/labels to keep the diff to find-and-replace plus URL family migration, while merging `Provider` into the [`Clinician`](../clinicians/clinician.py) model is a non-trivial structural change (FK rename across `affiliations`, the three `provider_*` credential tables, `owner_id` transfer) with no user-visible benefit. A follow-up issue may pull the model classes together if/when it becomes worth it.

Reading rule of thumb: a reference to "Provider" in code or this README means the Python class; a reference to "Clinician" in the UI / URL / audit-log means the same row read through its post-rename surface. The single declaration that bridges them is [`../../specs/provider.py`](../../specs/provider.py), where `name="clinician"` + `url_collection="clinicians"` + `audit_action_stem="provider"` map the new vocabulary onto the existing model and audit-action enums (the enum values `CREATE_PROVIDER` / `UPDATE_PROVIDER` / `DELETE_PROVIDER` are pinned so historical audit rows keep their labels; the new `audit.type` records the post-rename string).
...
**Attaching a Provider to an Org.** The clinician create/edit form picks from a dropdown of the **Orgs the requesting user owns** (superusers see every Org). The wire enforces the same boundary: `POST /clinicians` and `PATCH /clinicians/{id}` reject an `org_id` that points at an Org the requesting user doesn't own — 403 if it exists, 404 if it doesn't (no leak of other users' Org ids). A user who needs to attach to an Org they don't own must first be granted ownership (invite/grant flow not yet built; for now, create your own Org via `/organizations/form`).
