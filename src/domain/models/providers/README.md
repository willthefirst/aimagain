...
Every Provider belongs to exactly one Org, and `provider.org.name` is the practice's display name. Post-#635 PR B `provider.org_id` is a `@property` over `provider.affiliation.org_id` — the FK actually lives on `affiliations.org_id` (NOT NULL, RESTRICT). Templates, schemas, and tests read `provider.org.name` (also a property over `affiliation.org.name`); there is no separate `practice_name` column.
