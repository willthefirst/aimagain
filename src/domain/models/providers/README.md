...
- `provider.py` — `Provider`. Tied to a `User` via non-unique `owner_id` FK + CASCADE (the original `uq_provider_profiles_user_id` was dropped in `8f20a93effc9` to allow multiple `Provider` rows per user). Credential sub-rows are now person-level (FK to `clinicians.id`, #635 PR A); `provider.licensures` / `.educations` / `.certifications` survive as `@property` proxies that delegate to `provider.clinician.licensures` etc. Appending into the proxy returns SQLAlchemy's `InstrumentedList` so the framework's `repo.add_child(parent, "licensures", row)` continues to set `row.clinician_id` automatically. Enum columns CHECK against `US_STATES` and `LOCATION_AVAILABILITY_OPTIONS` from [`../enums.py`](../enums.py). `clinician_id` (NOT NULL, non-UNIQUE FK to `clinicians`) links each provider to the person behind it; `provider.npi` is a `@property` proxy to `provider.clinician.npi` after the column moved to `clinicians.npi` in #629 PR 1. The constructor auto-creates a fresh `Clinician` from any wire-side `npi` kwarg so the framework's generic create handler keeps building the model via `spec.model(**payload.model_dump())` unchanged. See [`../clinicians/clinician.py`](../clinicians/clinician.py) for the person-attribute home and the NPI format CHECK. The `(city, state, zip)` triple comes from [`LocationMixin`](../../../framework/persistence/mixins.py) — same mixin `ReferralDetail` uses, so the column declarations stay in lockstep. The `provider.affiliation` back-relationship (1:1, cascade-delete) points at the row in [`../affiliations/affiliation.py`](../affiliations/affiliation.py) — populated by #629 PR 2 with a mirror of the per-role columns (`org_id`, `location_*`, sessions, insurance posture, sliding scale, cost). PR 3 swaps the directory UI to read those columns through `provider.affiliation`; PR 4 drops the duplicates from this table.
...
- `provider.py` carries the user-facing `@property` proxies (`provider.licensures` etc.) so route handlers and templates keep their existing `provider.<credential>` shape; the actual `relationship(...)` declarations live on `Clinician` because credentials are person-level after #635 PR A.
