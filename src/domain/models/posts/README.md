...
- `opening_detail.py` — `OpeningDetail`, the detail row for `kind='clinician_opening'`. Thin by design after #1358 PR-f sub-3: carries only the announcement core (`schedule_text` / `description`) plus the two context FKs (`clinician_id`, `clinician_affiliation_id`). The per-announcement `subject` and `treatment_modality` overrides were dropped to align with the referral model — the headline is derived from the linked practice's org name, and modality reads from the affiliation's structured `modalities` list. The steady-state practice profile (`services` / `settings` / `modalities` / `age_groups` / `genders` / `languages` / `website` / `referral_instructions`) lives on the linked `ClinicianAffiliation` (and `languages` on the linked `Clinician`) — see [`../clinician_affiliations/README.md`](../clinician_affiliations/README.md). Table name stays `opening_details` from before the kind rename (migration `9e1f7b3c4a2d`) — pure rename noise to drop it.
- `intake_detail.py` — `IntakeDetail`, the detail row for `kind='program_intake'` (#541). Same thin shape as `OpeningDetail` (`schedule_text` / `description` minus the clinician context — Program is the only context FK; the same `subject` / `treatment_modality` drop applies, deriving the headline from the program name). The steady-state profile lives on the linked `Program` (`languages` included, since the intake side is program-level). Table name stays `intake_details` for the same reason as `opening_details`.
