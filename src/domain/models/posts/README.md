...
Pulling them into `posts/` makes the boundary explicit: a reader landing here finds the entire posts data model in one place. Cross-domain shared modules (the controlled-vocabulary tuples in `../enums.py`, the `BaseModel` in [`src/framework/base_model.py`](../../../framework/base_model.py)) stay at the parent level because they're consumed by the providers cluster too.
