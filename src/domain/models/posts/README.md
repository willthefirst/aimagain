...
- `referral_detail.py` — `ReferralDetail`, the detail row for `kind='referral'`. 1:1 with `posts` via `post_id` (PK + FK with CASCADE). Columns track the client-referral intake form; enum columns CHECK against tuples in [`../enums.py`](../enums.py); the `desired_times` and `services` JSON multi-selects have their vocabularies enforced on the wire by Pydantic (no SQL CHECK against array members). The `(city, state, zip)` triple comes from [`LocationMixin`](../../../framework/persistence/mixins.py) — same mixin `Provider` uses. `target_opening_id` (added in migration `c9f7e3b2a1d8`) is a nullable UUID that records which `OpeningDetail` post triggered the referral when created via the `/welcome/refer/{opening_id}` wizard step; it is `NULL` for referrals created through the full `/referrals` form. No FK constraint — opening rows may be deleted without cascading to referral history.
- `opening_detail.py` — `OpeningDetail`, the detail row for `kind='clinician_opening'`. Same shape as `ReferralDetail` but tracks the clinician-availability form; adds the `settings` JSON multi-select. `services` and `settings` are required-min-1 on the wire (vs. optional/absent on Client Referral). Table name stays `opening_details` from before the kind rename (migration `9e1f7b3c4a2d`) — pure rename noise to drop it. T3 (migration `af760f747e18`) added the slot-shape fields: `opening_type` (CHECK-constrained to `OPENING_TYPES`), `availability_state` (CHECK-constrained to `AVAILABILITY_STATES`), `specialties` / `population_tags` / `payment_types` (free-form JSON arrays), `fee_low` / `fee_high` (nullable integers), `colleague_note` (nullable text, ≤280 chars enforced by Pydantic), `note_expires_at` (nullable timestamp), and `last_confirmed_at` (non-null timestamp, defaults to `now()`). All new fields are optional on the wire so existing rows remain valid.
...
Before this extraction, `post.py`, `post_kinds.py`, and the two detail files lived alongside `provider.py`, `provider_licensure.py`, etc. as siblings of one flat `src/domain/models/`. They formed a cluster the directory didn't reflect:
