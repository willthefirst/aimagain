...
Inverse: delete the registry entry, the detail model file, the `relationship(...)` line on `Post`, the four Pydantic variant classes in `src/domain/logic/posts/schema.py`, the per-kind templates under `src/domain/templates/posts/`, and ship a migration that drops the detail table and narrows the CHECK. No edits in routes, repositories, or logic — see [`../../api/routes/posts.py`](../../api/routes/posts.py) and the dispatch ladders in [`../../logic/post_processing.py`](../../logic/post_processing.py); both read from the registry. The retired `note` kind (removed in migration `c2d3e4f5a6b7`) is the canonical example of how clean this is when the registry is the only source of truth.
