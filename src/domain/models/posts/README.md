...
- `referral_detail.py` — `ReferralDetail`, the detail row for `kind='referral'`. 1:1 with `posts` via `post_id` (PK + FK with CASCADE). Columns track the client-referral intake form; enum columns CHECK against tuples in [`../enums.py`](../enums.py); the `desired_times`, `services`, `affirming_identities`, and `acceptable_license_types` JSON multi-selects have their vocabularies enforced on the wire by Pydantic (no SQL CHECK against array members). The `age_groups` JSON column is the one exception that carries a SQL CHECK — not on *membership* but on *cardinality*: `ck_referral_details_age_groups_single` (`json_array_length(age_groups) <= 1`) because a referral describes a single client (exactly one bucket; the form renders a single `<select>` and the wire's `RequiredAgeGroupsField` is `min_length=1, max_length=1`). This is the *referral* sense of `age_groups` (single bucket); an opening's `age_groups` is multi-valued (on `OpeningDetail` itself, since an opening serves a cohort), and an intake's lives on the linked `Program`. `affirming_identities` is the request-side mirror of `Clinician.affirming_identities` — the referrer states the affordances the client needs, the clinician claims the ones they offer. `acceptable_license_types` is the license-class disjunction on the referred provider ("psychiatrist OR PMHNP"); empty list = "no constraint", reusing the same `LicenseType` vocabulary `ClinicianLicensure` writes from. The `clinical_niches` JSON column carries free-form request-side niche tags ("DGBI", "ADHD in women", "complex trauma"), the request-side mirror of `Clinician.clinical_niches`; deliberately **not** an enum (#1358 PR-c) — the niche vocabulary is too open-ended on day one. The wire validator `clean_free_form_tags` strips each tag, drops empties, and deduplicates. The `(city, state)` pair is declared **inline** (explicit `Text` columns, no ZIP) — a referral targets a city or area, not a postal address, so it does *not* use [`LocationMixin`](../../../framework/persistence/mixins.py). The `<field>_other_text` free-text columns (`services_other_text`, `insurance_carriers_other_text`, `languages_other_text`, `pronouns_other_text`) and the in-person `location_city` are **conditionally required** — the one home for which field is required-when is [`../../logic/posts/conditional_fields.py`](../../logic/posts/conditional_fields.py), which drives both the server-side validation (field-targeted 422s) and the form's pure-CSS `:has()` reveal. No SQL CHECK enforces these (the dependency is cross-field).
- `opening_detail.py` — `OpeningDetail`, the detail row for `kind='clinician_opening'`. **Self-describing announcement**: it carries its own per-announcement dimensions — delivery format (`session_format`), service lines (`services` / `services_other_text`, on the same `ReferralService` 12-leaf vocabulary the referral request side uses), the cohort it's for (`age_groups`, multi-valued — an opening serves a cohort, unlike referral's single-client cardinality CHECK — and `genders`), and `cost` — plus the announcement core (`schedule_text` / `description`) and the two context FKs (`clinician_id`, `clinician_affiliation_id`). The remodel **reversed** the #1358 PR-f split for these dimensions: they're per-announcement, not per-affiliation, so a clinician can post two openings under the same affiliation targeting different cohorts/formats. The headline is derived from the linked practice's org name (no custom `subject` override), aligning with the referral model. Only the steady-state context (location, insurance posture, `website` / `referral_instructions`, `currently_accepting_new_patients`) lives on the linked `ClinicianAffiliation` — see [`../clinician_affiliations/README.md`](../clinician_affiliations/README.md); `languages` lives on the linked `Clinician`. Table name stays `opening_details` from before the kind rename (migration `9e1f7b3c4a2d`) — pure rename noise to drop it.
- `intake_detail.py` — `IntakeDetail`, the detail row for `kind='program_intake'` (#541). **Thin** (unlike the now-self-describing `OpeningDetail`): carries the announcement core (`schedule_text` / `description`) plus the single Program context FK — no clinician context, and the headline is derived from the program name (no `subject` override). The steady-state profile lives on the linked `Program` (`languages` included, since the intake side is program-level). Table name stays `intake_details` for the same reason as `opening_details`.
