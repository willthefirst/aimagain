...
- `referral_detail.py` — `ReferralDetail`, the detail row for `kind='referral'`. 1:1 with `posts` via `post_id` (PK + FK with CASCADE). Columns track the client-referral intake form; enum columns CHECK against tuples in [`../enums.py`](../enums.py); the `desired_times`, `services`, and `affirming_identities` JSON multi-selects have their vocabularies enforced on the wire by Pydantic (no SQL CHECK against array members). `affirming_identities` is the request-side mirror of `Clinician.affirming_identities` — the referrer states the affordances the client needs, the clinician claims the ones they offer. The `(city, state, zip)` triple comes from [`LocationMixin`](../../../framework/persistence/mixins.py) — same mixin `Clinician` uses.
