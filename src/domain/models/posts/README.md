...
- `program_availability_detail.py` — `ProgramAvailabilityDetail`, the detail row for `kind='program_availability'` (#541). Same per-announcement field set as `OpeningDetail` but points at a `Program` via `program_id` instead of a `Provider` — the referrer is choosing an *intake door* (the Program) and trusting the owning Org to assign a clinician internally.
