...
`Clinician` exposes proxy `@property`/setter pairs for the affiliation's steady-state context columns (`org_id`, `location_city`/`_state`, `accepts_out_of_network`, `in_network_carriers`, `sliding_scale` — the `_PER_ROLE_ATTRS` tuple; no `location_zip`, and the per-announcement service/session/cohort/cost fields are not proxied because they live on the opening post, not the affiliation). They proxy reads from / writes to `primary_clinician_affiliation` (`clinician_affiliations[0]`). When no affiliation exists the **setters raise `ValueError`** rather than silently dropping the write — this catches the prod regression where an unaffiliated clinician's edit form returned 200 but persisted nothing. The readers still return `None` for backwards compatibility with templates that may render an unaffiliated row.
