...
Sub-PR 2 (read flip + dual-write) is now in. The view layer (`src/domain/logic/posts/view.py`) reads steady-state fields off this Program row when an intake announcement is rendered (with fallback to the detail row's column when the Program field is empty — the dual-write safety net). `PostRepository.create_polymorphic` / `patch` mirror non-empty fields from a freshly-created or patched `IntakeDetail` onto this row, keeping the two homes consistent until sub-PR 3 drops the detail-row columns.
