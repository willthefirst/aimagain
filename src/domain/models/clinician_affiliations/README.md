...
After #1308, practice posture is patched **directly on the affiliation**, not through the clinician's PATCH form: `ClinicianUpdate` is **person-level only** (identity + person-level claims like `affirming_identities`, `clinical_niches`, `languages`), and the affiliation owns the per-(clinician × org) practice posture. After the canonical-pattern conversion (#1402), each affiliation has its own list / new / edit pages under `/clinicians/{clinician_id}/clinician_affiliations[/...]`; the edit form surfaces the practice cluster (org, location, sessions, in-network, OON, sliding-scale, cost) and the steady-state practice profile (`services` / `settings` / `modalities` / `age_groups` / `genders` / `website` / `referral_instructions`) — i.e. every field on the wire schema's Update surface. `currently_accepting_new_patients` is not editable from the form: it's the server-managed cache (toggled by the OpeningDetail lifecycle), surfaced read-only on the list page as an "Accepting new patients" badge.
