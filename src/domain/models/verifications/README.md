...
One row per nightly verification attempt against a `Provider`. Records the outcome the orchestrator (#528) computed: a `status` from `VERIFICATION_STATUSES`, free-form `flags`, the raw NPPES response, an `oig_match` boolean, and an optional NPPES-vs-provider `name_match_score`. Parent-layer conventions (BaseModel inheritance, FK coverage, migrations) live in [`../README.md`](../README.md).
