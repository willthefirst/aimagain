...
Auto-bound handlers expose the same `_handle_<verb>_<entity>` attribute on the route module that hand-written bindings used to — contract tests patching `src.domain.routes.<entity>._handle_<verb>_<entity>` resolve through the mount layer's `_resolve_handler` either way.
