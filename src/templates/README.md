...
| **users/** | User management        | list, detail, `provider_profiles_list.html` (rendered by `GET /users/{id}/provider-profiles` and the `GET /users/me/provider-profiles` alias; pluralized list with self vs. viewing-other empty-state copy), `_admin_actions.html` partial (shared by list & detail) |
