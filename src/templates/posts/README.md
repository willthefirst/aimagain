...
- `_form_macros.html` — shared field-render macros (`text_field`, `textarea_field`, `select_field`, `radio_bool_field`, `time_grid_field`, `multi_select_field`). Used by the per-kind form partials below. The `<select>` and `multi_select_field` macros iterate over a controlled-vocabulary tuple from [`src/models/post_enums.py`](../../models/post_enums.py) (registered as Jinja globals in [`src/core/templating.py`](../../core/templating.py)) and look display labels up in a sibling `*_LABELS` dict. `time_grid_field` renders a 7×3 day-of-week × part-of-day checkbox grid; `multi_select_field` renders a flat checkbox list driven by any vocabulary tuple. Both use the same shared-`name` trick: stock `json-enc` serializes the checked values as a string (1 selected) or array (2+), and the schema's `_scalar_to_list` `BeforeValidator` normalizes the 1-element case to a list before validation.
