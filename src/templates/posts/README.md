...
- `_form_macros.html` — shared field-render macros (`text_field`, `textarea_field`, `select_field`, `radio_bool_field`). Used by the per-kind form partials below. The `<select>` macro iterates over a controlled-vocabulary tuple from [`src/models/post_enums.py`](../../models/post_enums.py) (registered as Jinja globals in [`src/core/templating.py`](../../core/templating.py)) and looks display labels up in a sibling `*_LABELS` dict.
