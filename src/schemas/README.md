...
- `provider_profile.py` - Provider directory profile (`ProviderProfile`) plus its credential sub-entities (`ProviderLicensure`, `ProviderEducation`, `ProviderCertification`), each with Read/Create/Update/AuditSnapshot variants. Reuses `StrippedText` and `ZipText` from `post.py`. Controlled-vocabulary fields use `Literal[*TUPLE]` against the tuples in [`src/models/post_enums.py`](../models/post_enums.py); the `test_schema_literals_match_model_tuples` guardrail in `test_provider_profile.py` keeps schema and model in lockstep.
