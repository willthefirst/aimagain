...
- `post.py` - Post schemas: discriminated unions (`PostCreate`, `PostRead`, `PostUpdate`) over per-kind schemas (`ClientReferralCreate`/`Read`/`Update`, `ProviderAvailabilityCreate`/`Read`/`Update`), plus `PostAuditSnapshot`. `kind` is the discriminator; per-kind schemas set `extra="forbid"`. `ClientReferral*` mirror the multi-section intake form (5 sections, ~12 fields including JSON list fields `desired_times` and `services`). Section-enum literals (`_State`, `_Availability`, `_AgeGroup`, `_LanguagePreferred`, `_Service`, `_Insurance`, `_DesiredTimeSlot`) are validated against the model-side tuples by `test_schema_literals_match_model_tuples`. Adding a new post kind = new per-kind schemas plus an entry in each Union.
...
- `test_post.py` — exercises the kind-discriminated `PostCreate` and `PostUpdate` unions: dispatch on `kind`, rejection of missing/unknown `kind`, per-kind required fields and validators (`client_referral` covers all 5 intake-form sections including the JSON multi-selects `desired_times` and `services`; `provider_availability` carries `specialty`/`region`/`accepting_new_clients`), whitespace stripping, the per-section enums (state/age/availability/insurance/etc.), 5-digit ZIP validation, multi-select duplicate rejection, the bool-from-string coercion, at-least-one-field on Update, the `extra="forbid"` boundary on each per-kind schema, and the `test_schema_literals_match_model_tuples` guardrail (keeps schema literals in lock-step with the model tuples that feed the DB CHECK constraints).
