...
- `post.py` - Post schemas: discriminated unions (`PostCreate`, `PostRead`, `PostUpdate`) over per-kind schemas (`ClientReferralCreate`/`Read`/`Update`, `ProviderAvailabilityCreate`/`Read`/`Update`), plus `PostAuditSnapshot`. `kind` is the discriminator; per-kind schemas set `extra="forbid"`. Adding a new post kind = new per-kind schemas plus an entry in each Union.
...
- `test_post.py` — exercises the kind-discriminated `PostCreate` and `PostUpdate` unions: dispatch on `kind`, rejection of missing/unknown `kind`, per-kind required fields and validators (`summary`/`urgency`/`region` on `client_referral`; `specialty`/`region`/`accepting_new_clients` on `provider_availability`), whitespace stripping, the urgency enum, the bool-from-string coercion, at-least-one-field on Update, and the `extra="forbid"` boundary on each per-kind schema.
