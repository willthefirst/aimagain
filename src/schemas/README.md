...
- `provider_profile.py` - Provider directory profile (`ProviderProfile`) plus its credential sub-entities (`ProviderLicensure`, `ProviderEducation`, `ProviderCertification`), each with Read/Create/Update/AuditSnapshot variants. Reuses `StrippedText` and `ZipText` from `_validators.py`. Controlled-vocabulary fields use `Literal[*TUPLE]` against the tuples in [`src/models/enums.py`](../models/enums.py); the `test_schema_literals_match_model_tuples` guardrail in `test_provider_profile.py` keeps schema and model in lockstep.
- `_validators.py` - Cross-module schema primitives (free-text cleaning aliases, ZIP regex, the unified at-least-one-field helper). New schema modules import from here instead of from a peer domain module; new primitives move here when a second schema module needs them.
...
- `test_provider_profile.py` — covers the four provider-profile entities (profile + licensure + education + certification): controlled-vocabulary rejection, `extra="forbid"` rejection, the at-least-one-field rule on each Update variant, `ProviderProfileRead.model_validate` round-trip on a nested dict, and the `test_schema_literals_match_model_tuples` guardrail aligning `Literal[*TUPLE]` types with `src/models/enums.py`.
