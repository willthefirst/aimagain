...
Schemas follow the [cluster pattern](../README.md#domain-entities-and-the-cluster-pattern): one cluster directory per entity, with `_validators.py` as the parent-level shared tier.

| Schema File | Domain    | Responsibilities                             | Schema Types                                             |
| ----------- | --------- | -------------------------------------------- | -------------------------------------------------------- |
| **users/user.py** | User data | User CRUD plus activation state-axis subresource (extends FastAPI Users) and audit snapshots | UserRead, UserCreate, UserUpdate, UserActivationUpdate, UserAuditSnapshot, UserActivationAuditSnapshot |
| **posts/post.py** | Posts     | `PostCreate`/`PostUpdate`/`PostRead`/`PostAuditSnapshot` are kind-discriminated unions keyed on the (required) `kind` field. Create/Update variants apply `extra="forbid"`, strip whitespace, validate ZIPs against `^\d{5}$`, type controlled-vocabulary fields as `Literal[*TUPLE]` against the tuples in [`src/models/enums.py`](../models/enums.py), type the multi-select fields as `DesiredTimesField` / `ServicesField` / `SettingsField` (each a `list[Literal[*TUPLE]]` annotated with a shared `_scalar_to_list` `BeforeValidator` that wraps a 1-element scalar back into a list — htmx's stock `json-enc` collapses a 1-checkbox-checked group to a string on the wire; default `[]` on Create, `None` = leave-unchanged on Update; PA's `services` and `settings` are `RequiredServicesField` and `RequiredSettingsField` — same aliases plus `Field(min_length=1)` so Create/Update reject empty lists while Read/AuditSnapshot stay tolerant of historic rows), and (for partial update) require at least one mutable field. Read and AuditSnapshot variants share a single `_flatten_post_to_dict` helper that reads the per-kind detail relationship and field tuple from `REGISTERED_KINDS` in [`src/models/posts/post_kinds.py`](../models/posts/post_kinds.py) — adding a kind there is what wires the flatten path up. `post_audit_snapshot(post)` validates a SQLAlchemy `Post` against the audit union and returns a JSON-mode dump. | ClientReferral*, ProviderAvailability* variants of Create/Update/Read/AuditSnapshot, plus the discriminated-union aliases PostCreate/PostUpdate/PostRead/PostAuditSnapshot and the helper `post_audit_snapshot` |
| **providers/provider.py** | Provider directory entry | Read/Create/Update/AuditSnapshot variants for `Provider` and its three credential sub-entities (`ProviderLicensure`, `ProviderEducation`, `ProviderCertification`). Create/Update apply `extra="forbid"`, reuse `StrippedText` and `ZipText` from `_validators.py`, and type controlled-vocabulary fields (`license_type`, `education_type`, `certification_type`, `issuing_state`/`location_state`, `in_person_sessions`/`virtual_sessions`) as `Literal[*TUPLE]` against the tuples in [`src/models/enums.py`](../models/enums.py). Update variants enforce at-least-one-field via the shared `assert_any_field_set` helper from `_validators.py`. `ProviderRead` and `ProviderAuditSnapshot` embed the sub-entity lists; `ProviderUpdate` does **not** — credential lists are managed via their own routes (added in later issues). | ProviderRead/Create/Update/AuditSnapshot, ProviderLicensure*, ProviderEducation*, ProviderCertification* variants of Read/Create/Update/AuditSnapshot |
| **\_validators.py** | Cross-module schema primitives (parent-level shared tier) | Home for `Annotated[T, AfterValidator(fn)]` aliases and small helpers used by 2+ schema modules. Today: `StrippedText`, `StrippedOptionalText`, `ZipText` (free-text cleaning + ZIP regex) and `assert_any_field_set(model, *, exclude=...)` (the partial-update at-least-one-field rule, with optional-discriminator support via `exclude`). Add to it when a primitive is used by 2+ schema modules (rule of three, applied generously when callers would otherwise import across a domain boundary), or when two modules are about to define near-duplicates of the same helper. | `StrippedText`, `StrippedOptionalText`, `ZipText`, `assert_any_field_set` |

## Directory structure

**Cluster directories** (one per entity; tests colocated):

- `users/user.py` + `users/test_user.py` (none today) - User schemas extending FastAPI Users base schemas
- `posts/post.py` + `posts/test_post.py` - Per-kind post schemas wrapped in discriminated-union aliases (`PostCreate`/`PostUpdate`/`PostRead`/`PostAuditSnapshot`) keyed on the required `kind` field. The Read and AuditSnapshot variants share one `_flatten_post_to_dict` helper that reads from `REGISTERED_KINDS` in [`src/models/posts/post_kinds.py`](../models/posts/post_kinds.py); adding a new kind means a registry entry there plus the four Pydantic variant classes here. Controlled-vocabulary fields use `Literal[*TUPLE]` against the tuples in [`src/models/enums.py`](../models/enums.py); the `test_schema_literals_match_model_tuples` guardrail in `test_post.py` keeps schema and model in lockstep.
- `providers/provider.py` + `providers/test_provider.py` - Provider directory entry (`Provider`) plus its credential sub-entities (`ProviderLicensure`, `ProviderEducation`, `ProviderCertification`), each with Read/Create/Update/AuditSnapshot variants. Reuses `StrippedText` and `ZipText` from `_validators.py`. Controlled-vocabulary fields use `Literal[*TUPLE]` against the tuples in [`src/models/enums.py`](../models/enums.py); the `test_schema_literals_match_model_tuples` guardrail in `test_provider.py` keeps schema and model in lockstep.

**Parent-level shared tier:**

- `_validators.py` - Cross-module schema primitives (free-text cleaning aliases, ZIP regex, the unified at-least-one-field helper). New schema modules import from here instead of from a peer cluster; new primitives move here when a second schema cluster needs them.
