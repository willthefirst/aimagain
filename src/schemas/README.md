...
- `test_provider.py` — covers the four provider entities (profile + licensure + education + certification): controlled-vocabulary rejection, `extra="forbid"` rejection, the at-least-one-field rule on each Update variant, `ProviderRead.model_validate` round-trip on a nested dict, and the `test_schema_literals_match_model_tuples` guardrail aligning `Literal[*TUPLE]` types with `src/models/enums.py`.
