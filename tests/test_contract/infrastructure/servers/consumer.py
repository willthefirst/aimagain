...
def _setup_clinician_create_form_stub(app: FastAPI) -> None:
    ...
    async def clinician_create_form_stub_page(request: Request):
        ...
        return APIResponse.html_response(
            ...
            # `spec.create_adapter`.
            # `entity_name` + `create_heading` are required by the
            # `views/form_new.html` chrome contract (#1271) — the
            # production handler injects them via `mount_form`;
            # bespoke stubs that render the same template must too.
            context={
                "current_user": current_user,
...
def _setup_clinician_edit_form_stub(app: FastAPI) -> None:
    ...
    async def clinician_edit_form_stub_page(request: Request, clinician_id: uuid.UUID):
        ...
        return APIResponse.html_response(
            ...
            # `entity_name` + `edit_heading` + `resource_detail_url` are
            # required by the `views/form_edit.html` chrome contract
            # (#1271) — the production handler injects them via
            # `mount_form`; bespoke stubs that render the same template
            # must too.
            context={
                "clinician": clinician,
                "entity_name": "clinician",
                "edit_heading": "Edit clinician",
                "resource_detail_url": f"/clinicians/{clinician_id}",
                "current_user": current_user,
                "orgs": [org],
            },
            request=request,
...
def _setup_organization_create_form_stub(app: FastAPI) -> None:
    ...
    async def organization_create_form_stub_page(request: Request):
        ...
        return APIResponse.html_response(
            ...
            # `entity_name` + `create_heading` satisfy the
            # `views/form_new.html` chrome contract (#1271).
            context={
                "current_user": current_user,
