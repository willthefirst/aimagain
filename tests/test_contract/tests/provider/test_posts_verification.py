"""Provider verification: POST /posts and DELETE /posts/{id} accept the
...
server fixture (auth-mocked). `handle_create_post` and `handle_delete_post`
...
PATCH /posts/{id} is intentionally absent — there are no editable fields
in this PR, so there is no edit form and no edit verification. They return
when per-kind fields land.

One module-scoped provider server with both mocks applied — every pact
