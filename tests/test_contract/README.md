...
> **Status:** auth (registration), users (admin actions), and posts (owner actions) currently have contract test pairs. Per-kind create/edit-form pairs for the remaining post kinds are deferred until their form schemas stabilize. Add a pair for any new HTML form (or htmx-driven action partial) per the conventions below.
...
└── tests/
    ...
    │   └── test_posts_verification.py   # Verifies post owner-actions pact
    └── shared/
