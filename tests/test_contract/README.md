...
> **Status:** auth (registration), users (admin actions), and posts (create form, owner actions) currently have contract test pairs. Add a pair for any new HTML form (or htmx-driven action partial) per the conventions below.
...
└── tests/
    ...
    │   └── test_posts_verification.py   # Verifies post create + owner-actions pacts
    └── shared/
